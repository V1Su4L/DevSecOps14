...
users = [
    {"id": 1, "username": "hodi", "password": "123",
        "salary": 5000, "address": "123Main St", "active": True},
    {"id": 2, "username": "josh", "password": "456",
        "salary": 60000, "address": "456 Oak St", "active": True}
]
...
# all the get http requests with id as int will run this function
...
def get_user_by_id(user_id):
    ...
    # if the user_id doesnt exist return 404 status code
...
def delete_by_id(user_id):
    for user in users:
        if user.get('id') == user_id:
            # replace the users with new list of userts that dont have the same id as the user_id param  TODO: [user for user in users if user['id']!= user_id ]
            users.remove(user)
